...
def analyse_sales_performance(test=False):
    ...
    latest_inventory.to_parquet('data/outputs/latest_inventory_value.parquet', compression='gzip')
...
def analyse_character_needs(character=[]):
    ...
    df = pd.read_parquet('data/intermediate/inventory.parquet')
...
def analyse_item_min_sell_price(MIN_PROFIT_MARGIN=1000, MAT_DEV=0.5, test=False):
    ...
    if test: return None # avoid saves
    item_min_sale[['min_list_price']].to_parquet('data/intermediate/min_list_price.parquet', compression='gzip')
...
def analyse_sell_data(test=False):
    ...
    auction_scan_minprice = pd.read_parquet('data/intermediate/auction_scan_minprice.parquet')
...
def apply_sell_policy(stack_size=1, leads_wanted=15, duration='medium', update=True, factor=1, test=False, leave_one=True):
    ...
    if update and not test:
        df_sell_policy.to_parquet('data/outputs/sell_policy.parquet', compression='gzip')
