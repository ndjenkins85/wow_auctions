...
def generate_auction_scandata(test=False):
    ...
    auction_scan_minprice_repo = pd.read_parquet('data/full/auction_scan_minprice.parquet')

    if test: return None # avoid saves    
    auction_scan_minprice.to_parquet('data/intermediate/auction_scan_minprice.parquet', compression='gzip')
    auction_scan_minprice_repo.to_parquet('data/full_backup/auction_scan_minprice.parquet', compression='gzip')

    updated = '*not*'
    if auction_scan_minprice['timestamp'].max() > auction_scan_minprice_repo['timestamp'].max():
        updated = ''
        auction_scan_minprice_repo = pd.concat([auction_scan_minprice, auction_scan_minprice_repo], axis=0)
...
def generate_booty_data():
    ...
    """
    account = '396255466#1'
    pricerdata = utils.read_lua("Pricer", merge_account_sources=False, accounts=[account])[account]["PricerData"]
